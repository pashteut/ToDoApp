...
import com.pashteut.todoapp.todo_api.TodoApi
import com.pashteut.todoapp.common.AppDispatchers
import com.pashteut.todoapp.data.functions.getDeviceId
import com.pashteut.todoapp.data.functions.toElementDTO
import com.pashteut.todoapp.data.functions.toToDoItem
import com.pashteut.todoapp.data.functions.toTodoItem
import com.pashteut.todoapp.data.functions.toTodoItemDBO
...
class TodoItemsRepository @Inject constructor(
    ...
    fun getAllItems() = database.observeAll().map {list -> list.map { item -> item.toTodoItem() } }
    ...
    private suspend fun _saveItem(item: TodoItemDBO, saveToCache: Boolean = true) =
        withContext(appDispatchers.io) {
    ...
    suspend fun saveItem(item: TodoItem) = _saveItem(item.toTodoItemDBO())

    private suspend fun _updateItem(item: TodoItemDBO) =
        withContext(appDispatchers.io) {
    ...
    suspend fun updateItem(item: TodoItem) = _updateItem(item.toTodoItemDBO())
